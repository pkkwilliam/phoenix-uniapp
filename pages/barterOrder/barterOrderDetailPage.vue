<template>
  <view
    class="safearea-container sticky-bottom-container-padding"
    v-if="loaded"
  >
    <application-navigation-bar title="我買到的" />
    <view class="step-container custom-navbar-temp-spacer">
    ...
    <view class="cu-bar foot">
      <view class="bottom-container full-width">
...
export default {
  ...
  computed: {},
  data() {
    return { barterOrder: undefined, loaded: false };
  },
  methods: {
  ...
  async onLoad(option) {
    ...
    await this.getBarterOrder(barterOrderId);
    this.loaded = true;
